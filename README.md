...
### 7. **세트상품 관리 시스템** 🆕
- **원가 비율 배분**: 세트상품 매출을 구성품 원가 비율에 따라 자동 분배
- **웹 기반 에디터**: 직관적인 UI로 세트상품 등록/수정/삭제
- **자동 재고 분해**: 세트상품 판매 시 개별 구성품으로 자동 분할 처리
- **원가 자동 계산**: 구성품의 원가를 기반으로 총 원가 자동 산출
- **쿠팡 매핑 연동**: 세트상품 이름 변경 시 쿠팡 매핑 자동 업데이트

### 8. **실패 로그 자동 생성** 🆕
- **자동 복구 정보**: 업로드 실패 시 `실패기록/` 폴더에 상세 로그 자동 생성
- **배치 번호 추적**: 실패한 정확한 배치 번호 기록
- **재개 가이드**: 실패 지점부터 재시작할 수 있는 명령어 자동 제공
- **데이터 소스별 관리**: 이지어드민/쿠팡 데이터 분리 기록
...
# 3. 누락건 중간 배치부터 업로드 🆕
python main.py fix
# → 실패한 배치부터 재시작 (실패기록 파일 참고)

# 4. 세트상품 관리 🆕
python main.py
# → 메뉴에서 "5) 세트상품 관리" 선택
# → 웹 브라우저: http://localhost:5002

# 또는 직접 실행
python set_product_editor.py
# → 세트상품 등록/수정/삭제 웹 UI

# 5. 쿠팡 상품 매핑 에디터 (수동)
python coupang_product_editor.py
# → http://localhost:5001
...
├── 실패기록/                     # 업로드 실패 로그 자동 저장 폴더 🆕
├── main.py                       # 메인 진입점 (완전한 워크플로우)
├── excel_converter.py            # 엑셀 변환 + 데이터 검증
├── seller_mapping.py             # 판매처 매핑 DB 관리 (MySQL + GPT 통합)
├── seller_editor.py              # 판매처 수동 매핑 웹 에디터 (Flask, 포트 5000)
├── coupang_rocketgrowth.py       # 쿠팡 로켓그로스 데이터 처리 🆕
├── coupang_product_mapping.py    # 쿠팡 상품 매핑 DB 관리 (세트상품 포함) 🆕
├── coupang_product_editor.py     # 쿠팡 상품 수동 매핑 웹 에디터 (Flask, 포트 5001) 🆕
├── set_product_editor.py         # 세트상품 관리 웹 에디터 (Flask, 포트 5002) 🆕
├── fix_product_names.py          # 상품명 일괄 수정 유틸리티
...
### 개요
세트상품(번들 상품)을 개별 구성품으로 자동 분해하여 정확한 재고 및 매출 관리를 지원합니다.

### 주요 기능
- **원가 비율 배분**: 세트상품 매출을 각 구성품의 원가 비율에 따라 자동 분배
- **웹 기반 관리**: 직관적인 UI로 세트상품 등록/수정/삭제
- **자동 DB 연동**: 세트상품 변경 시 쿠팡 매핑 테이블 자동 업데이트
...
브라우저에서 `http://localhost:5002` 자동 오픈

#### 2. 세트상품 등록
1. **세트상품명** 입력 (예: "닥터시드 비타민 3종 세트")
2. **브랜드** 선택 (닥터시드/딸로/테르스/에이더)
3. **구성 상품 선택**:
   - 개별 상품 선택 (스탠다드 상품 목록에서)
   - 수량 지정 (예: 비타민C × 2개, 비타민D × 1개)
4. **저장** → 완료 페이지로 이동

#### 3. 매출 분배 로직
세트상품 판매 시 자동으로 구성품으로 분해:

```
세트상품 매출: 100,000원
구성품:
  - 비타민C (원가 3,000원) × 2개 = 6,000원 (비중 60%)
  - 비타민D (원가 2,000원) × 2개 = 4,000원 (비중 40%)

자동 배분:
  - 비타민C 매출: 60,000원, 수량: 2개
  - 비타민D 매출: 40,000원, 수량: 2개
```

#### 4. 쿠팡 매핑 연동
세트상품 이름 변경 시:
- `coupang_product_mapping` 테이블 자동 업데이트
- 기존 쿠팡 옵션 매핑 유지
- 삭제 시 관련 매핑 자동 정리

### 데이터베이스 구조

#### set_products (세트상품 정의)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | INT | PRIMARY KEY |
| set_name | VARCHAR(500) | 세트상품명 (UNIQUE) |
| brand | VARCHAR(100) | 브랜드 |
| created_at | TIMESTAMP | 생성일시 |

#### set_product_items (세트 구성품)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | INT | PRIMARY KEY |
| set_id | INT | 세트상품 ID (FK) |
| standard_product_name | VARCHAR(500) | 구성품 상품명 |
| quantity | INT | 구성 수량 |
...
### 자동 실패 로그 생성
업로드 중 실패 시 자동으로 상세 로그 생성:

#### 저장 위치
```
실패기록/
└── 실패기록_20251121_143052.txt
```
...
1. 실패 원인을 확인하고 수정하세요
2. 다음 명령어로 실패한 배치부터 재시작하세요:

   python main.py fix

3. 프로그램 실행 후 배치 번호 입력:
   - 첫 번째 실패 배치: 2
   - 또는 전체 재시도
...
### v1.5.0 (Latest) 🆕
- ✨ **세트상품 관리 시스템**: 원가 비율 기반 매출 자동 분배
- ✨ **웹 기반 세트상품 에디터**: 직관적인 UI로 세트상품 등록/수정/삭제 (포트 5002)
- ✨ **자동 재고 분해**: 세트상품 판매 시 개별 구성품으로 분할 처리
- ✨ **실패 로그 자동 생성**: 업로드 실패 시 상세 복구 정보 자동 저장 (`실패기록/` 폴더)
- ✨ **배치 복구 기능**: 실패한 배치부터 재시작 가능
- ✨ **쿠팡 상품 수동 매핑 에디터**: GPT 실패 시 수동 매핑 웹 UI (포트 5001)
- 🐛 세트상품 이름 변경 시 쿠팡 매핑 자동 업데이트
- 🐛 세트상품 삭제 시 연관 매핑 자동 정리
